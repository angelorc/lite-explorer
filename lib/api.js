...
class Api {
  ...
  getLatestBlocks() {
    return fetch(`${this.url}/blocks?offset=0&limit=5`).then(response => response.json())
  }

  getLatestTxs() {
    return fetch(`${this.url}/transactions?offset=0&limit=5`).then(response => response.json())
  }
  ...
  getUptime(valoper, start, end, with_blocks = false) {
    return fetch(`${this.url}/validators/${valoper}/uptime?start=${start}&end=${end}&with_blocks=${with_blocks}`).then(response =>
      response.json()
    )
  }
}
