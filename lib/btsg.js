...
class Btsg {
  ...
  getBalanceByDenom(address, denom) {
    return fetch(`${this.url}/cosmos/bank/v1beta1/balances/${address}/${denom}`).then(response =>
      response.json()
    ).then(response =>
      response.balance
    )
  ...
  getTotalSupply() {
    return fetch(`${this.url}/cosmos/bank/v1beta1/supply/ubtsg`).then(response =>
      response.json()
    )
  }
  ...
  getValidators(status = 'BOND_STATUS_BONDED') {
