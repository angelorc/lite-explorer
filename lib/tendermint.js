...
class Client {
  ...
  unsubscribe(subscription_id) {
    ...
    return new Promise((resolve, reject) => {
      const obj = self.subscriptions.findIndex(o => o.id === subscription_id)
      if (this.closed) return

      const params = convertWsArgs(self.subscriptions[obj].args)

      this.ws.send(
        JSON.stringify({ jsonrpc: `2.0`, id: subscription_id, method: 'unsubscribe', params })
      )

      self.subscriptions.splice(obj, 1)
