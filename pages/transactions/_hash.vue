<template>
  <page-template>
    ...
    <v-card class="mb-8">
      <v-container>
        <v-row>
          ...
          <v-col cols="12" md="2" :class="{'py-0': $vuetify.breakpoint.smAndDown }" class="grey--text text--darken-2">Tx Hash</v-col>
          <v-col cols="12" md="10" :class="{'pt-0': $vuetify.breakpoint.smAndDown }">
            {{ tx.tx_hash }}
          </v-col>
          <v-col cols="12" md="2" :class="{'py-0': $vuetify.breakpoint.smAndDown }" class="grey--text text--darken-2">Status</v-col>
          <v-col cols="12" md="10" :class="{'pt-0': $vuetify.breakpoint.smAndDown }">
            <div v-if="tx.logs">
              <v-icon left color="green">mdi-check-bold</v-icon> Succes
            </div>
            <div v-else>
              <v-icon color="red" left>mdi-alert-circle</v-icon> <span class="red--text">{{ tx.error_log }}</span>
            </div>
          </v-col>
          ...
          >
          <v-col cols="12" md="10" :class="{'pt-0': $vuetify.breakpoint.smAndDown }">{{ tx.gas_used }} / {{ tx.gas_wanted }}</v-col>
          <v-col cols="12" md="2" :class="{'py-0': $vuetify.breakpoint.smAndDown }" class="grey--text text--darken-2">Signature</v-col>
          <v-col cols="12" md="10" :class="{'pt-0': $vuetify.breakpoint.smAndDown }">
          ...
          <v-col v-if="tx.memo" cols="12" md="2" :class="{'py-0': $vuetify.breakpoint.smAndDown }" class="grey--text text--darken-2">Memo</v-col>
          <v-col v-if="tx.memo" cols="12" md="10" :class="{'pt-0': $vuetify.breakpoint.smAndDown }">
            {{ tx.memo }}
          </v-col>
        </v-row>
      </v-container>
    ...
    <transaction-messages class="mb-8" :msgs="tx.messages"></transaction-messages>
